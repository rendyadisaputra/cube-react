...
class CubeStoreDriver extends query_orchestrator_1.BaseDriver {
    ...
    async importStreamingSource(columns, tableData, table, indexes, uniqueKeyColumns) {
        if (!uniqueKeyColumns) {
        ...
        await this.query(`CREATE SOURCE OR UPDATE ${this.quoteIdentifier(tableData.streamingSource.name)} as ? VALUES (${Object.keys(tableData.streamingSource.credentials).map(k => `${k} = ?`)})`, [tableData.streamingSource.type]
            .concat(Object.keys(tableData.streamingSource.credentials).map(k => tableData.streamingSource.credentials[k])));
        const createTableSql = this.createTableSql(table, columns);
        // eslint-disable-next-line no-unused-vars
