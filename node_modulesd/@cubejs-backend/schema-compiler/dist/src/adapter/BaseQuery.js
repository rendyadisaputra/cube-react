...
class BaseQuery {
    ...
    get dataSource() {
        ...
        if (dataSources.length > 1 && !this.externalPreAggregationQuery()) {
            throw new UserError_1.UserError(`To join across data sources use rollupJoin with Cube Store. Note: this is an experimental feature for different data source types. Found data sources: ${dataSources.join(', ')}`);
        }
            ...
            get: (target, name) => {
                ...
                return new Proxy({ cube: cubeName }, {
                    get: (cubeNameObj, propertyName) => {
                        const filters = allFilters.filter(f => f.dimension === this.cubeEvaluator.pathFromArray([cubeNameObj.cube, propertyName]));
                        return {
                            filter: (column) => {
                                if (!filters.length) {
                                    return '1 = 1';
